...
# ---------------------------------------------------------------------------
# RM3 pseudo-relevance feedback expansion (Move M)
# ---------------------------------------------------------------------------


def test_rm3_expand_query_adds_frequent_content_terms():
    """RM3 appends top-frequency content terms from first-pass hits."""
    from types import SimpleNamespace

    from ai_knot.query_runtime import _rm3_expand_query

    hits = [
        SimpleNamespace(raw_text="Caroline moved from Sweden to the United States"),
        SimpleNamespace(raw_text="Caroline grew up in Sweden for many years"),
        SimpleNamespace(raw_text="Her home country Sweden has good schools"),
    ]
    question = "Where did Caroline move from?"

    expanded = _rm3_expand_query(question, hits, top_m_terms=3)

    assert expanded.startswith(question), "Must keep original question as prefix"
    assert "sweden" in expanded.lower(), "Most frequent content term must be appended"


def test_rm3_expand_query_excludes_query_and_stop_terms():
    """RM3 must skip stopwords and tokens already present in the question."""
    from types import SimpleNamespace

    from ai_knot.query_runtime import _rm3_expand_query

    hits = [
        SimpleNamespace(raw_text="Caroline is happy where she lives"),
        SimpleNamespace(raw_text="Caroline is happy with her friends"),
    ]
    # "happy" appears in both hits and the question — must NOT be re-appended.
    question = "What makes Caroline happy?"

    expanded = _rm3_expand_query(question, hits, top_m_terms=5)
    tail = expanded[len(question) :].split()

    assert "happy" not in tail, "Tokens from the question must not be re-appended"
    assert "is" not in tail, "Stopwords must be excluded"
    assert "the" not in tail, "Stopwords must be excluded"


def test_rm3_expand_query_empty_hits_returns_question():
    """With no hits, expansion is a no-op."""
    from ai_knot.query_runtime import _rm3_expand_query

    assert _rm3_expand_query("any question", []) == "any question"
