...
All method calls are safe to run concurrently — the client multiplexes them over a single subprocess using JSON-RPC 2.0 request IDs:
...
Save and restore the complete state of the knowledge base:

```typescript
await kb.add('User prefers Python');
await kb.snapshot('before-refactor');

await kb.add('User switched to Go');

await kb.restore('before-refactor');  // rolls back
```

Snapshots require SQLite or YAML storage (not available on all backends).

---

## Lifecycle

The subprocess starts lazily on the first method call and is reused for all subsequent calls. Always call `kb.close()` when done to cleanly shut it down:
...
## Environment variables (alternative to constructor options)

The subprocess reads these env vars if constructor options are not set:
